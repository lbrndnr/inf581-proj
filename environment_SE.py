...
class environment:
    ...
    """
    def __init__(self, maze_size=15, initial_snake_size=5, mice_points=[1,2,3], fixed_obstacles=[(3,4),(3,5),(3,6)]):
    ...
    def print_maze(self):
        ...
        for i in range(-1, self.maze_size + 1):
            ...
            for j in range(-1, self.maze_size + 1):
                if (i,j) in self.fixed_obstacles or i < 0 or i >= self.maze_size or j < 0 or j >= self.maze_size:
                    print ('#', end='')
                elif (i,j) in self.obstacles:
                    print ('@', end='')
                elif (i,j) in self.mice:
                    print (self.mice[(i,j)], end='')
                else:
                    print (' ', end='')